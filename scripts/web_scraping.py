...
from tqdm import (
    tqdm,
)  # ajoute une bar de progression pour connaître le temps de chargement
from parsel import Selector  # pour extraire des données HTML
from nested_lookup import (
    nested_lookup,
)  # permet de rechercher des clés spécifiques dans des strucures de données imbriquées
from playwright.sync_api import (
    sync_playwright,
)  # sert pour naviguer automatiquement dans des pages web
...
def scrape_thread(url: str, max_pages: int) -> dict:
    ...
    for page_num in range(1, max_pages + 1):
        with sync_playwright() as pw:
            ...
            with tqdm(
                total=max_pages, desc="Scraping en cours ☻", colour="magenta"
            ) as pbar:
                for page_num in range(1, max_pages + 1):
                    try:
